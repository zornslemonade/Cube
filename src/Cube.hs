...
instance TwistyPuzzle CubeConfiguration (CenterP, EdgeP, VertexP) (CenterO, EdgeO, VertexO) Cubie where
  getPositions :: CubeConfiguration -> (CenterP, EdgeP, VertexP)
  getPositions (Cube (a, b, c, xs, ys, zs)) = (a, b, c)
...
instance TwistablePuzzle CubeConfiguration (CenterP, EdgeP, VertexP) (CenterO, EdgeO, VertexO) Cubie Turn where
  applyTurn :: Turn -> CubeConfiguration -> CubeConfiguration
  applyTurn = \case
    I -> id
    U -> (u |#|)
    U' -> (invert u |#|)
...
isSimilarTo (Cube (a, b, c, xs, ys, zs)) (Cube (a', b', c', xs', ys', zs')) = t1 && t2 && t3 && t4 && t5
  where
    ...
    t3 = sgn b * (-1) ^ unmod (F.fold xs) == sgn b' * (-1) ^ unmod (F.fold xs')
    t4 = F.fold ys == F.fold ys'
    t5 = F.fold zs == F.fold zs'
...
-- sgn b = (-1)^(F.fold xs) (For each pair of edge cubies swapped, a center cubie is turned once and vice versa)
-- [(-1)^(F.fold xs) since F.fold xs is in Z_4]
-- F.fold ys = 0 (Edge cubies are flipped in pairs)
-- F.fold zs = 0 (Vertex cubies are turned in opposite pairs--if one is turned cw, another is turned ccw)
...
solve g
  | isLegal g = Just $ solve' g
  | otherwise = Nothing
  where
    -- The centers are never out of position in a legal configuration, so no repositioning is required
    ...
    solve' g0 =
      let ms1 = orientCenters $ invert g0
          g1 = applyTurns g0 ms1
          ms2 = positionEdges $ invert g1
          g2 = applyTurns g1 ms2
          ms3 = orientEdges $ invert g2
          g3 = applyTurns g2 ms3
          ms4 = positionVertices $ invert g3
          g4 = applyTurns g3 ms4
          ms5 = orientVertices $ invert g4
          g5 = applyTurns g4 ms5
          ms6 = orientLastCenter $ invert g5
